...
class BLTSeq2SeqModel(nn.Module):
    ...
    def forward(
        ...
        logits = self.local_decoder(tgt_bytes, global_dec_output)
        return logits
    ...
    def greedy_decode(
        ...
        for patch_idx in range(max_patches):
            ...
            # Run global decoder on all generated bytes so far
            global_dec_output = self.decode_patches(generated, enc_output, src_pad_mask)

            # Get the last patch's global representation
            last_patch_repr = global_dec_output[:, -1:, :]  # (batch, 1, d_model)

            # Generate bytes for this patch autoregressively
            patch_bytes = torch.full((batch_size, 1), BYTE_BOS, dtype=torch.long, device=device)
            ...
            # Append generated patch bytes (skip the BOS we added for the local decoder)
            new_bytes = patch_bytes[:, 1:]  # (batch, patch_size)
            generated = torch.cat([generated, new_bytes], dim=1)

        # Remove initial BOS and any trailing PAD
        result = generated[:, 1:]
        return result
